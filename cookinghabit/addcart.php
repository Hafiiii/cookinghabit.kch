...
if ($result->num_rows > 0) {
    ...
    $cartSql = "SELECT * FROM cart WHERE cart_session_id = $sessionId";
    $cartResult = $conn->query($cartSql);

    if ($cartResult->num_rows > 0) {
        // Product is already in the database cart, update the quantity
        $updateSql = "UPDATE cart SET cart_quantity = cart_quantity + $quantity WHERE product_id = $productId AND cart_session_id = $sessionId";
        $conn->query($updateSql);
    } else {
        // Product is not in the database cart, add it as a new item
        $insertSql = "INSERT INTO cart (product_id, cart_quantity, cart_price, cart_image, cart_name, cart_session_id) 
                VALUES ($productId, $quantity, $productPrice, '$productImage', '$productName', '$sessionId')";
